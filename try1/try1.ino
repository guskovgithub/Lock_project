...
int CurPosx = 0, CurPosy = 0; //две переменные хранящие текущее положение курсора
...
void setup()
{
    lcd.init();
    lcd.backlight();
    initialize();//вызываем функцию очищающую дисплей
...
{  
     char keypressed = myKeypad.getKey();
      if (keypressed != NO_KEY) { //если на кейпаде что то нажато
        ...
        if (CurPosy == 0) { //здесь мы определяем на какой позиции пароля или логина мы сейчас находимся и заполняем массивы а также выводим символы на экран
